...
原创 bleeping
                    bleeping  暗镜   2026-02-03 04:00  
  
乌克兰计算机应急响应小组 (CERT) 表示，俄罗斯黑客正在利用 CVE-2026-21509，这是微软 Office 多个版本中最近修复的一个漏洞。  
  
1 月 26 日，微软发布了紧急带外安全更新，将 CVE-2026-21509 标记为正在被积极利用的零日漏洞。  
  
在微软发出警报仅仅三天后，CERT-UA 就检测到了利用该漏洞传播的恶意 DOC 文件，这些文件以欧盟驻乌克兰常驻代表委员会磋商为主题。  
  
在其他情况下，这些电子邮件冒充乌克兰水文气象中心，并发送到 60 多个政府相关地址。  
  
然而，该机构表示，与该文件相关的元数据显示，该文件是在紧急更新发布一天后创建的。  
  
乌克兰 CERT 将这些攻击归咎于 APT28，这是一个国家级威胁行为体，也称为 Fancy Bear 和 Sofacy，与俄罗斯总参谋部情报总局 (GRU) 有关联。  
...
![](https://mmbiz.qpic.cn/mmbiz_png/mibm5daOCSt9q0Jiat2q2QQJeTW7FqrZQvdiaicTdPj51YTU0sMVa0cbZy8zLBP6rTCtAeqKcVIlN2pPDgAz5n4JQw/640?wx_fmt=png&from=appmsg "")  
  
CERT-UA 在报告中指出：“计划任务的执行导致 explorer.exe 进程终止并重新启动，而 explorer.exe 进程通过 COM 劫持等方式确保加载“EhStoreShell.dll”文件。”  
  
“该 DLL 从映像文件中执行 shellcode，从而确保在计算机上启动 COVENANT 软件（框架）。”  
...
如果无法立即进行修补，建议按照我们 最初 对该漏洞的报道中提供的基于注册表的缓解措施说明进行操作。  
  
微软此前曾表示，Defender 的受保护视图通过阻止来自互联网的恶意 Office 文件（除非明确信任）来增加额外的防御层。  
